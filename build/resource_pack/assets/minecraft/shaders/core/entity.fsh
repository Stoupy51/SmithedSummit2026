...
#endif
#ifdef DISSOLVE
    float dissolveMask = texture(DissolveMaskSampler, texCoord0).r;
    float dissolveThreshold = 1.0 - ColorModulator.a;
    if (dissolveMask < dissolveThreshold) {
        discard;
    }
    float dissolveEdge = 0.05;
    if (dissolveMask < dissolveThreshold + dissolveEdge) {
        float edgeFactor = 1.0 - (dissolveMask - dissolveThreshold) / dissolveEdge;
        color.rgb = mix(color.rgb, vec3(0.0, 0.5, 1.0), edgeFactor * 0.8);
    }
#endif

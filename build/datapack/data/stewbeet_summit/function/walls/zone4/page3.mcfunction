
#> stewbeet_summit:walls/zone4/page3
#
# @within	stewbeet_summit:walls/zone4/show
#

data modify entity 20180612-2026-2002-2098-202000000004 text set value ["", {"text": "Ores that spawn (4/6)\n\n", "bold": true, "color": "gold"}, {"text": "Make the ore generate in the world\n", "color": "white"}, {"text": "with ", "color": "white"}, {"text": "CustomOreGeneration", "bold": true, "color": "aqua"}, {"text": ":\n\n", "color": "white"}, {"text": "CustomOreGeneration(\n", "color": "#8BE9FD"}, {"text": "    dimensions=[\"minecraft:overworld\"],\n", "color": "#8BE9FD"}, {"text": "    maximum_height=70,\n", "color": "#8BE9FD"}, {"text": "    veins_per_region=4,\n", "color": "#8BE9FD"}, {"text": "    vein_size_logic=0.4,\n", "color": "#8BE9FD"}, {"text": ")\n\n", "color": "#8BE9FD"}, {"text": "Controls dimensions, height & vein size.\nVia Smart Ore Generation lib.", "color": "#7F8C99", "italic": true}]
data modify entity 20180612-2026-2002-2098-202000000004 line_width set value 220
data modify entity 20180612-2026-2002-2098-202000000004 transformation.scale set value [0.45, 0.45, 0.45]
data modify entity 20180612-2026-2002-2098-202100000004 item.components."minecraft:item_model" set value "stewbeet_summit:nav_arrow_left"
data modify entity 20180612-2026-2002-2098-202200000004 item.components."minecraft:item_model" set value "stewbeet_summit:nav_arrow_right"


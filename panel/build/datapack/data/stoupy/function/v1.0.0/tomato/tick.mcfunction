...
#
# @within	stoupy:v1.0.0/tick [ as @e[type=item_display,tag=stoupy.tomato] & at @s ]
#
...
function #bs.move:apply_vel {scale: 0.001, with: {blocks: true, entities: false, on_collision: "function stoupy:v1.0.0/tomato/splat"}}
...
scoreboard players remove @s stoupy.tomato.life 1
execute if score @s stoupy.tomato.life matches ..0 run function stoupy:v1.0.0/tomato/splat

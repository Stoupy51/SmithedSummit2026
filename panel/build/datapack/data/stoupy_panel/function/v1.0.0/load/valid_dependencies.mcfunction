...
# Decode errors
execute if score #mcload_error stoupy_panel.data matches 1 run tellraw @a {"text":"Boost Your Productivity Error: This version is made for Minecraft 26.2+.","color":"red"}
execute if score #dependency_error stoupy_panel.data matches 1 run tellraw @a {"text":"Boost Your Productivity Error: Libraries are missing\nplease download the right Boost Your Productivity datapack\nor download each of these libraries one by one:","color":"red"}
execute if score #dependency_error stoupy_panel.data matches 1 unless score $bs.move.major load.status matches 4.. run tellraw @a {"text":"- [Bookshelf Move (v4.1.0+)]","color":"gold","click_event":{"action":"open_url","url":"https://github.com/mcbookshelf/bookshelf/releases"}}
execute if score #dependency_error stoupy_panel.data matches 1 if score $bs.move.major load.status matches 4 unless score $bs.move.minor load.status matches 1.. run tellraw @a {"text":"- [Bookshelf Move (v4.1.0+)]","color":"gold","click_event":{"action":"open_url","url":"https://github.com/mcbookshelf/bookshelf/releases"}}
